...
export class pPlanBreakdown extends React.Component {
  ...
  componentDidMount() {
    ...
    console.log('pPlanBreakdown componentDidMount planSelected = ', planSelected);
    ...
    planBreakdown((response) => {
      ...
      // console.log('pPlanBreakdown planDetail = ', map(payLoad, (r) => r.planResults));
    }, JSON.stringify(configData), doMailState, startDate);

  }

  componentWillUnmount() {
    console.log('pPlanBreakdown unmounting');
  ...
  _handleDimChange = ({ window }) => {
    ...
    // console.log('pPlanBreakdown _handleDimChange event, new flag  = ', flag);
    this.setState({
      flag: flag,
      adjust: adjust
    })
  ...
  _renderInnerRowDrug = (info) => {
    ...
    // console.log('pPlanBreakdown _renderInnerRowDrug ncCost = ', ncCost);
  ...
  _getItemLayout = (data, index) => (
    {
      length: Platform.OS === 'web' ? Dimensions.get('window').width / 2 : Dimensions.get('window').width,
      offset: (Platform.OS === 'web' ? Dimensions.get('window').width / 2 : Dimensions.get('window').width) * index,
      index
    }
  )
  ...
  _renderContent(currPlan, index) {
    ...
    return (
      ...
      <View style={{ width: Platform.OS === 'web' ? Dimensions.get('window').width / 2 : Dimensions.get('window').width, alignItems: 'stretch' }}>
  ...
  render() {
    ...
    //console.log('pPlanBreakdown render currPlan = ', currPlan);
    if (!planDetail) { return null; }

    return (
      <View style={{ height: Dimensions.get('window').height - 75 - (adjust ? 0 : 35), borderColor: 'red', borderWidth: 0 }} >
...
export default connect(mapStateToProps, mapDispatchToProps)(pPlanBreakdown);
